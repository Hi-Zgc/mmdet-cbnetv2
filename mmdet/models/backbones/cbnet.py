...
from .eca_resnet import ECAResNet
...
# 注册自己的网络CBAM-ResNet
class _CBAMResNet(_CBSubnet, CBAMResNet):
    def __init__(self, **kwargs):
        _CBSubnet.__init__(self)
        CBAMResNet.__init__(self, **kwargs)


class _CBCBAMResNet(_CBNet):
    def __init__(self, net, cb_inplanes, cb_zero_init=True, cb_del_stages=0, **kwargs):
        super(_CBCBAMResNet, self).__init__()
        self.cb_zero_init = cb_zero_init
        self.cb_del_stages = cb_del_stages
...
class CBCBAMResNet(_CBCBAMResNet):
    def __init__(self, **kwargs):
        super().__init__(net=_CBAMResNet, **kwargs)

# 注册自己的网络ECA-ResNet
class _ECAResNet(_CBSubnet, ECAResNet):
    def __init__(self, **kwargs):
        _CBSubnet.__init__(self)
        ECAResNet.__init__(self, **kwargs)

class _CBECAResNet(_CBNet):
    def __init__(self, net, cb_inplanes, cb_zero_init=True, cb_del_stages=0, **kwargs):
        super(_CBECAResNet, self).__init__()
        self.cb_zero_init = cb_zero_init
        self.cb_del_stages = cb_del_stages
...
@BACKBONES.register_module()
class CBECAResNet(_CBECAResNet):
    def __init__(self, **kwargs):
        super().__init__(net=_ECAResNet, **kwargs)
    ...
    def forward(self, x, cb_feats=None, pre_tmps=None):
        ...
        for i in range(self.num_layers):
            ...
            if layer is None:
                ...
                if cb_feats is not None:
                    x = x + cb_feats[i]
                # 按照swin SwinBlockSequence 输出进行接收
                hw_shape = (Wh, Ww)
                x, down_hw_shape, x_out, hw_shape = layer(x, hw_shape)
                # down_hw_shape = tuple(down_hw_shape)
                # hw_shape = tuple(hw_shape)
